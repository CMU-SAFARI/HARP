 ...
 *
 * @brief [Adapted from the original BEER project] Routines that interface the
 * HARP artifacts with the EINSim simulator
 *
...
std::string einsim_generate_ecc_code(
      const std::string &einsim_binary
    , const uint64_t desired_k
    , const uint64_t desired_p);
