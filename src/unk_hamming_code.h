 ...
 *
 * @brief [Adapted from the original BEER project] Representation of an unknown
 * Hamming code and associated Z3 processing routines
 *
...
private:
    ...
    void build_ecc_code_standard_form_with_syndromes_ordered(void);
...
public:
     ...
     */
    unk_hamming_code(z3::context &z3_ctx, z3::solver &z3_solver, uint64_t k, bool syndromes_ordered=false) 
        : ctx(z3_ctx), sol(z3_solver)
    {
        this->K = k;
        ...
        auto start_time = std::chrono::high_resolution_clock::now();
        if(syndromes_ordered)
            build_ecc_code_standard_form_with_syndromes_ordered();
        else
    ...
    void solver_set_param(const char *s, unsigned val) const { z3::params z3_params(ctx); z3_params.set(s, val); sol.set(z3_params); } /**< set a z3 param */
    void solver_reset(void) const { sol.reset(); } /**< reset all constraints in the z3 solver */
    void solver_push(void) const { sol.push(); } /**< save the current state of the z3 solver */
    void solver_pop(void) const { sol.pop(); } /**< restore the state of the z3 solver */
    ...
    Eigen::Matrix< gf2, 1, Eigen::Dynamic > calculate_syndrome_from_error_pattern(
    ...
    void calculate_corrected_dataword_from_error_mask_given_zero_codeword(
          const Eigen::Matrix< gf2, 1, Eigen::Dynamic > &error_mask
        , Eigen::Matrix< gf2, 1, Eigen::Dynamic > &corrected_codeword
        , Eigen::Matrix< gf2, 1, Eigen::Dynamic > &dataword_p
        , Eigen::Matrix< gf2, 1, Eigen::Dynamic > &syndrome
    ...
    bool find_ecc_function(
        ...
        , const bool is_rerun);
    void find_ecc_function_impl(int n_errs_pre_correction, int max_n_errors_post_correction);

    bool find_possible_postcorrection_errors(
          const std::unordered_set< uint64_t > &precorrection_errors
        , std::unordered_set< uint64_t > &postcorrection_errors) const;
    bool find_beep_pattern(
    ...
    bool get_histogram_of_n_simultaneous_errors(
          const std::unordered_set< uint64_t > &precorrection_errors
        , const std::unordered_set< uint64_t > &postcorrection_errors
        , const std::unordered_set< uint64_t > &known_errors
        , uint64_t &max_n_unknown_posc_errors) const;
        // , std::unordered_set< uint64_t > &histogram) const;
